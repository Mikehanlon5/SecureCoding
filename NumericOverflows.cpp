...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    {
        //Check to see if an addition will exceed bounds of type limit, if yes return 0 to signal error to test_overflow
        if ((std::numeric_limits<T>::max() - result) < increment) {
            std::cout << std::endl << "\tOverflow Imminent:" << +(result + increment) << std::endl;
            return 0;
        }
        result += increment;
    }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        // If the minimum bound of a type + the decrement is greater than current result, then the next subtraction will cause underflow
        if ((std::numeric_limits<T>::min() + decrement) > result) {
            std::cout << std::endl << "\tUnderflow Imminent" << +(result - decrement) << std::endl;
            return 0;
        }
        result -= decrement;
    }
...
void test_overflow()
{
    ...
    //Extra logic to deduce whether overflow occured or was prevented
    if (result == 0) {
        std::cout << "\tOverflow Prevented" << std::endl;
    }
    else {
        std::cout << "Overflow Occured: " << +result << std::endl;
    }
...
void test_underflow()
{
    ...
    //Extra logic to deduce whether underflow occured or was prevented
    if (result == 0) {
        std::cout << "\tUnderflow Prevented" << std::endl;
    }
    else {
        std::cout << "Underflow Occured: " << +result << std::endl;
    }
}
